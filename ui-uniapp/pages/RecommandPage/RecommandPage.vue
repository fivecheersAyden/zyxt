...
const saveDialogue = ()=>{
	...
	uni.getStorage({
		...
		success(loginStorage) {
			...
			uni.request({
				url: globalProps.baseApi + 'question?type=exercise',
				method: "POST",
...
const getRecommandQues = ()=>{
	...
	uni.request({
		url: globalProps.baseApi + 'exercise/recommend/v3?userID=' + id.value,
		header: {
		...
		success(res) {
			...
			if(res.statusCode === 200){
				if(res.data.length < 1){
					...
					uni.showToast({
						icon: 'none',
						title:'暂无推荐习题，请先做题完善用户画像'
					})
					return
				}
				for (let s of res.data) {
					if(s.category === 'yuedu'){
						s.cuoTiList = s.sonExercise
					}
				}
				recommandQuesList.value = res.data
				updateCurrentQues()
				console.log('获取推荐习题成功', recommandQuesList.value)
				console.log(JSON.stringify(recommandQuesList.value))
			}else{
				uni.hideLoading()
				console.log('获取推荐习题失败',res)
				uni.showToast({
					icon: 'error',
					title: '获取推荐习题失败'
				})
